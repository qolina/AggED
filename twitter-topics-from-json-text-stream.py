		...
		if (tweet_unixtime - tweet_unixtime_old) < time_window_mins * 60:
			...
			if len(users) < 3 and len(hashtags) < 3 and len(features) > 3 and len(tweet_bag.split(",")) > 4 and not str(features).upper() == str(features):
				...
				# time interval filtering
                                hour = datetime.fromtimestamp(tweet_unixtime_old).strftime("%H")
                                mins = datetime.fromtimestamp(tweet_unixtime_old).strftime("%M")
                                twStr = str(int(hour))+"_"+str(int(mins))
                                print "\nWindow Starts GMT Time:", dtime, twStr, len(window_corpus), time.asctime(), "\n"

				tweet_unixtime_old = tweet_unixtime	
                                #if twStr not in chosenTimeWindow: 
                                #    window_corpus = []
                                #    tids_window_corpus = []
                                #    tid_to_urls_window_corpus = {}
                                #    tid_to_raw_tweet = {}
